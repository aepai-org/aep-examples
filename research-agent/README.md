...
AGENT_CARD_URL=https://your-domain.example/.well-known/agent-card.json \
  node examples/research-agent/register.mjs
```
...
record. It does not verify research quality or execute a task.

For the G17.3 public lifecycle, set `AEP_API_KEY` and comma-separated
`AEP_CAPABILITY_IDS`, then run `node examples/research-agent/lifecycle.mjs`.
